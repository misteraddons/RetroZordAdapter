...
const byte PSX1_CMD = 16;
const byte PSX1_DAT = 14;
const byte PSX1_CLK = 15;
const byte PSX1_ATT = 2;
const byte PSX2_ATT = 5;
const byte PSX3_ATT = 10;
const byte PSX4_ATT = A0;
const byte PSX5_ATT = A1;
const byte PSX6_ATT = A2;
...
PsxControllerHwSpi<PSX1_ATT> psx1;
PsxControllerHwSpi<PSX2_ATT> psx2;
//PsxControllerHwSpi<PSX3_ATT> psx3;
//PsxControllerHwSpi<PSX4_ATT> psx4;
//PsxControllerHwSpi<PSX5_ATT> psx5;
//PsxControllerHwSpi<PSX6_ATT> psx6;
...
void init_jogcon() {
  if (psx1.begin()) {
    ...
    if (!psx1.enterConfigMode ()) {
      ...
      psx1.enableAnalogSticks ();
      //must enable rumble to use the jogcon's motor
      psx1.enableRumble ();
      psx1.exitConfigMode ();
    }
    psx1.read ();    // Make sure the protocol is up to date
  }
}
...
void readGuncon() {
  ...
  const GunconStatus gcStatus = psx1.getGunconCoordinates(x, y); //use coords from guncon
...
*/
  ...
  if (gcStatus == GUNCON_OK) {
    ...
    else if (psx1.buttonPressed(PSB_CIRCLE) && psx1.buttonPressed(PSB_START) && psx1.buttonPressed(PSB_CROSS)) {//only when using guncon. dualshock wont work
      enableReport = false;
      releaseAllButtons();
      delay(1000);
    }
...
void readDualShock() {//dualshock as guncon.
  ...
  if (psx1.getLeftAnalog(analogX, analogY)) { //use coords from analog controller
  ...
  if (enableJoystick) {
    ...
    if (psx1.getRightAnalog(analogX, analogY)) { //use coords from analog controller
      //analogDeadZone(analogX);
      //analogDeadZone(analogY);
    }
    //x = convertRange(ANALOG_MIN_VALUE, ANALOG_MAX_VALUE, analogX);
  ...
  */
  ...
  /*if (psx1.buttonPressed(PSB_SELECT)) {
      enableReport = false;
      releaseAllButtons();
      delay(1000);
  }*/
...
void handleDpad(const bool isjog = false) {
  int16_t dpad = B0;
  bitWrite(dpad, 0, !psx1.buttonPressed (PSB_PAD_UP));
  bitWrite(dpad, 1, !psx1.buttonPressed (PSB_PAD_DOWN));
  bitWrite(dpad, 2, !psx1.buttonPressed (PSB_PAD_LEFT));
  bitWrite(dpad, 3, !psx1.buttonPressed (PSB_PAD_RIGHT));
...
void handleButtons() { //guncon
  /*if (psx1.buttonJustPressed(PSB_CIRCLE)) { //trigger press
    AbsMouse->pressButton(0);//MOUSE_LEFT
    usbStick[0]->pressButton(0);
  }
  else if (psx1.buttonJustReleased(PSB_CIRCLE)) { //trigger release
    AbsMouse->releaseButton(0);//MOUSE_LEFT
    usbStick[0]->releaseButton(0);
  }

  if (psx1.buttonJustPressed(PSB_START)) { //A button press
    AbsMouse->pressButton(1);//MOUSE_RIGHT
    usbStick[0]->pressButton(1);
  }
  else if (psx1.buttonJustReleased(PSB_START)) { //A button release
    AbsMouse->releaseButton(1);//MOUSE_RIGHT
    usbStick[0]->releaseButton(1);
  }

  if (psx1.buttonJustPressed(PSB_CROSS)) { //B button press
    AbsMouse->pressButton(2);//MOUSE_MIDDLE
    usbStick[0]->pressButton(2);
  }
  else if (psx1.buttonJustReleased(PSB_CROSS)) { //B button release
    AbsMouse->releaseButton(2);//MOUSE_MIDDLE
    usbStick[0]->releaseButton(2);
  }*/
  
  const bool triggerPressed = psx1.buttonPressed(PSB_CIRCLE);
  const bool aBtnPressed = psx1.buttonPressed(PSB_START);
  const bool bBtnPressed = psx1.buttonPressed(PSB_CROSS);
...
void runCalibration() {
  if (calibrationStep == 1) { //calibrate x axis
    if (psx1.buttonJustPressed(PSB_START)) //A button press
      offsetX--;
    else if (psx1.buttonJustPressed(PSB_CROSS)) //B button press
      offsetX++;
  ...
  word x, y;
  if (psx1.getGunconCoordinates(x, y) == GUNCON_OK) {
...
void loopNeGcon() {
  byte lx, ly;
  psx1.getLeftAnalog (lx, ly);

  ((Joy1_*)usbStick[0])->setAnalog0(lx); //steering
  
  //uses inversed values
  ((Joy1_*)usbStick[0])->setAnalog1((uint8_t) ~ psx1.getAnalogButton(PSAB_L1)); //z
  ((Joy1_*)usbStick[0])->setAnalog2((uint8_t) ~ psx1.getAnalogButton(PSAB_CROSS)); //I throttle
  ((Joy1_*)usbStick[0])->setAnalog3((uint8_t) ~ psx1.getAnalogButton(PSAB_SQUARE)); //II brake

  if(isNeGconMiSTer)
    ((Joy1_*)usbStick[0])->setAnalog4(lx); //paddle 

  //combine the two axes and use only half precision for each
  //const int btnI_II = psx1.getAnalogButton(PSAB_SQUARE) - psx1.getAnalogButton(PSAB_CROSS);
  //const uint8_t btnI_II = ((psx1.getAnalogButton(PSAB_SQUARE) - psx1.getAnalogButton(PSAB_CROSS)) >> 1) + ANALOG_IDLE_VALUE;

  ((Joy1_*)usbStick[0])->setButton(0, psx1.buttonPressed(PSB_CIRCLE)); //A
  ((Joy1_*)usbStick[0])->setButton(1, psx1.buttonPressed(PSB_TRIANGLE)); //B
  ((Joy1_*)usbStick[0])->setButton(2, psx1.buttonPressed(PSB_R1));
  ((Joy1_*)usbStick[0])->setButton(3, psx1.buttonPressed(PSB_START)); //Start
  
  //uint8_t hat = (psx1.getButtonWord() >> 4) & B00001111;
...
void loopGuncon() {
  ...
  if (!enableReport) {
    if (!enableMouseMove && !enableJoystick) { //will only configure here on the first time.
        ...
        if (psx1.buttonJustPressed(PSB_CIRCLE)) { //trigger
          enableReport = true;
          enableMouseMove = true;
          //delay(300);
          return;
        } else if (psx1.buttonJustPressed(PSB_START)) { //A - left
          enableReport = true;
          enableJoystick = true;
          //delay(300);
          return;
        } else if (psx1.buttonJustPressed(PSB_CROSS)) { //B - right
    ...
    } else if (psx1.buttonJustPressed(PSB_CIRCLE) || psx1.buttonJustPressed(PSB_START)) { //re-enable the configured report mode
      enableReport = true;
      //delay(300);
      return;
    } else if (psx1.buttonJustPressed(PSB_CROSS)) { //enter calibration mode
      enableReport = true;
      calibrationStep = 1;
      delay(300);
      return;
  ...
  if (enableReport) {
    handleButtons();
    PsxControllerProtocol proto = psx1.getProtocol();
    switch (proto) {
    case PSPROTO_GUNCON:
      readGuncon();
      break;
    ...
    case PSPROTO_DUALSHOCK2:
      ...
      /*
      //controller buttons
      usbStick[0]->setButton (0, psx1.buttonPressed (PSB_SQUARE));
      // shared with gun: 1 PSB_CROSS; 2 PSB_CIRCLE, 9 PSB_START;
      usbStick[0]->setButton (3, psx1.buttonPressed (PSB_TRIANGLE));
      usbStick[0]->setButton (4, psx1.buttonPressed (PSB_L1));
      usbStick[0]->setButton (5, psx1.buttonPressed (PSB_R1));
      usbStick[0]->setButton (6, psx1.buttonPressed (PSB_L2));
      usbStick[0]->setButton (7, psx1.buttonPressed (PSB_R2));
      usbStick[0]->setButton (8, psx1.buttonPressed (PSB_SELECT));
      usbStick[0]->setButton (10, psx1.buttonPressed (PSB_L3));
      usbStick[0]->setButton (11, psx1.buttonPressed (PSB_R3));*/
      break;
    default:
      return;
    }
...
void loopDualShock() {
  ...
  case PSPROTO_FLIGHTSTICK:
    ...
    //controller buttons
    ((Joy1_*)usbStick[0])->setButton (0, psx1.buttonPressed (PSB_SQUARE));
    ((Joy1_*)usbStick[0])->setButton (1, psx1.buttonPressed (PSB_CROSS));
    ((Joy1_*)usbStick[0])->setButton (2, psx1.buttonPressed (PSB_CIRCLE));
    ((Joy1_*)usbStick[0])->setButton (3, psx1.buttonPressed (PSB_TRIANGLE));
    ((Joy1_*)usbStick[0])->setButton (4, psx1.buttonPressed (PSB_L1));
    ((Joy1_*)usbStick[0])->setButton (5, psx1.buttonPressed (PSB_R1));
    ((Joy1_*)usbStick[0])->setButton (6, psx1.buttonPressed (PSB_L2));
    ((Joy1_*)usbStick[0])->setButton (7, psx1.buttonPressed (PSB_R2));
    ((Joy1_*)usbStick[0])->setButton (8, psx1.buttonPressed (PSB_SELECT));
    ((Joy1_*)usbStick[0])->setButton (9, psx1.buttonPressed (PSB_START));
    ((Joy1_*)usbStick[0])->setButton (10, psx1.buttonPressed (PSB_L3));
    ((Joy1_*)usbStick[0])->setButton (11, psx1.buttonPressed (PSB_R3));

    //analog sticks
    if (psx1.getLeftAnalog(analogX, analogY)) {
    ...
    if (psx1.getRightAnalog(analogX, analogY)) {
      //convertedX = convertRange(ANALOG_MIN_VALUE, ANALOG_MAX_VALUE, analogX);
      //convertedY = convertRange(ANALOG_MIN_VALUE, ANALOG_MAX_VALUE, analogY);
...
void handleJogconData()
{
  ...
  JogconState jogStatus = psx1.getJogconState(jogPosition, jogRevolutions);

  newcnt = (jogRevolutions << 8) | jogPosition;//(data[5] << 8) | data[4];
  newbtn = psx1.getButtonWord();//(data[3] << 8) | data[2];
  ...
  if(jogStatus == JOGCON_STATE_OTHER) {//(data[0] == 0xF3)
    ...
    if(psx1.buttonPressed(PSB_L2) && psx1.buttonPressed(PSB_R2))
      mode = 0;
    else if(psx1.buttonPressed(PSB_L2))
      mode = 1;
    else if(psx1.buttonPressed(PSB_R2))
      mode = 2;


    // Force Feedback adjust
    // by pressing "mode" button while holding /\,O,X,[]
    if(psx1.buttonPressed(PSB_TRIANGLE))
      force = 1;
    else if(psx1.buttonPressed(PSB_CIRCLE))
      force = 3;
    else if(psx1.buttonPressed(PSB_CROSS))
      force = 7;
    else if(psx1.buttonPressed(PSB_SQUARE))
      force = 15;

    // Spinner pulses per step adjust
    // by pressing "mode" button while holding up,right,down,left
    if(psx1.buttonPressed(PSB_PAD_UP))
      sp_step = 1;
    else if(psx1.buttonPressed(PSB_PAD_RIGHT))
      sp_step = 2;
    else if(psx1.buttonPressed(PSB_PAD_DOWN))
      sp_step = 4;
    else if(psx1.buttonPressed(PSB_PAD_LEFT))
      sp_step = 8;

    // Paddle range switch by pressing "mode" button while holding:
    // L1 - 270 degree
    // R1 - 135 degree
    if(psx1.buttonPressed(PSB_L1) || psx1.buttonPressed(PSB_R1)) { //L 2, R 1
      sp_div = psx1.buttonPressed(PSB_L1) ? 2 : 1;
    ...
    jogStatus = psx1.getJogconState(jogPosition, jogRevolutions);
    ...
    if (ff == 1)
      psx1.setJogconMode(JOGCON_MODE_HOLD, force);
    else 
      psx1.setJogconMode(JOGCON_MODE_STOP, 0);
...
void psxSetup() {
  if (psx1.begin ()) {
    delay(200);
    haveController = true;
    const PsxControllerProtocol proto = psx1.getProtocol();
    ...
    if (proto == PSPROTO_GUNCON) {
      ...
      //Configure NeGcon mode
        ...
        if (psx1.buttonPressed(PSB_CIRCLE)) //NeGcon A / Volume B
      ...
      if (psx1.buttonPressed(PSB_SELECT)) { //dualshock used in guncon mode to help map axis on emulators.
        isGuncon = true;
      } else if (psx1.buttonPressed(PSB_L1)) {
        isJogcon = true;
      } else if (psx1.buttonPressed(PSB_L2)) {
        isJogcon = true;
        enableMouseMove = true;
  ...
  if (isNeGcon) {
    ...
    //enableMouseMove = haveController && psx1.buttonPressed(PSB_SELECT);
...
psxLoop() {
  ...
  if (millis() - last >= POLLING_INTERVAL) {
    ...
    if (!haveController) {
      ...
      noInterrupts();
      boolean isReadSuccess = psx1.read();
      interrupts();
